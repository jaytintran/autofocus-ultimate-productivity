...
export async function addTask(text: string, pageNumber: number, position: number): Promise<Task> {
  const supabase = createClient()
  const { data, error } = await supabase
    .from('tasks')
    .insert({
...
export async function addMultipleTasks(tasks: Array<{ text: string; pageNumber: number; position: number }>): Promise<Task[]> {
  const supabase = createClient()
  const { data, error } = await supabase
    .from('tasks')
    .insert(
...
export async function completeTask(id: string, totalTimeMs: number): Promise<Task> {
  const supabase = createClient()
  const { data, error } = await supabase
    .from('tasks')
    .update({
...
export async function reenterTask(originalTaskId: string, newText: string, pageNumber: number, position: number): Promise<Task> {
  const supabase = createClient()
  const { data, error } = await supabase
    .from('tasks')
    .insert({
...
export async function stopTimer(taskId: string, sessionTimeMs: number): Promise<{ appState: AppState; task: Task }> {
  const supabase = createClient()
...
export async function reenterFromPanel(taskId: string, taskText: string): Promise<Task> {
  const supabase = createClient()
  
  // Get the max position on the last page
  const activeTasks = await getActiveTasks()
  const maxPage = activeTasks.length > 0 
...
export async function reorderTasks(updates: Array<{ id: string; page_number: number; position: number }>): Promise<void> {
  const supabase = createClient()
...
export async function markTaskDone(taskId: string, totalTimeMs: number = 0): Promise<Task> {
  const supabase = createClient()
  const { data, error } = await supabase
    .from('tasks')
    .update({
